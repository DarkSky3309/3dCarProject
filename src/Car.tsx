...
const Auto: FC<{
  carOption: AUTO;
  camera: any;
  setSelectedDetail: SetStateAction<any>;
}> = ({ carOption, camera, setSelectedDetail }) => {
  ...
  useEffect(() => {
    async function load() {
      let model = await new GLTFLoader().loadAsync(
        process.env.PUBLIC_URL + `models/${carOption}/scene.gltf`
      );
      return model;
    }

    load()
      .then((r) => (auto.current = r))
  ...
  useFrame((state) => {
    if (!isLoaded) return;
    let group = auto.current.scene.children[0].children[0].children[0];
  ...
  useFrame(() => {
    ...
    if (intersects.length == 0 && nearObject !== null)
      return setNearObject(null);
    if (
      intersects.length == 0 ||
      nearObject?.object.parent.name === intersects[0].object.parent?.name
    )
      return;
    setNearObject(() =>
  ...
  return <>{isLoaded && <primitive object={auto.current.scene} />}</>;
};
